...
class Arc:
	...
	def __init__(self, input_stream, shiftx, shifty):
		...
		if not math.isinf(self.y) and not math.isinf(self.x) and not math.isnan(self.x) and not math.isnan(self.y):
			self.r = math.trunc(((self.x-xA) * (self.x-xA) + (self.y-yA) * (self.y-yA))**0.5)
		else:
			self.r = 0

		a = math.atan2(yA-self.y, xA-self.x)/math.pi*10.0*180.0
...
class Design:
	...
	def __init__(self, input_stream):
		...
		if len(tmp) > 5:
			tmp = ''.join(tmp[5:])
		self.shiftx = int(self.shiftx)*MULT
